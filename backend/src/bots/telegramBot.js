...
/**
 * 🚨 IMPORTANT FIX
 * Do NOT pass { polling: false }
 * This allows webhook updates to work correctly
 */
...
bot.on("error", (error) => {
  console.error("⚠️ Bot Error:", error);
});
...
function testTelegramConnection() {
  return new Promise((resolve, reject) => {
    const req = https.request(
      ...
      (res) => {
        res.statusCode === 200 ? resolve(true) : reject();
      }
    );
...
export async function setupTelegramWebhook(app) {
  ...
  try {
    ...
    console.error("❌ Telegram API unreachable");
  }
  ...
  console.log(`🎯 Webhook endpoint ready: POST ${webhookPath}`);

  if (!IS_PRODUCTION || !WEBHOOK_URL) {
    console.log("⚠️ Skipping webhook setup (not production)");
    return;
  }
...
bot.on("location", (msg) => {
  ...
  if (user) {
    ...
    user.lng = msg.location.longitude;
  ...
  saveUsers(users);
  bot.sendMessage(msg.chat.id, "📍 Location saved. Alerts enabled.");
});
...
bot.onText(/\/panic/, (msg) => {
  ...
  if (!sender) {
    return bot.sendMessage(msg.chat.id, "❌ Share your location first.");
  }
  ...
  users.forEach(user => {
    if (user.chatId === msg.chat.id) return;

    const dist = distanceKm(sender.lat, sender.lng, user.lat, user.lng);
    if (dist <= 5) {
      bot.sendMessage(
        user.chatId,
        `🚨 *PANIC ALERT*
...
🗺 https://sentinelai-rvy7.onrender.com`,
  ...
  bot.sendMessage(
    msg.chat.id,
    `🚨 Alert sent successfully.
${count} nearby user${count !== 1 ? "s" : ""} notified.`
  );
});
...
bot.on("message", async (msg) => {
  ...
  const wait = await bot.sendMessage(msg.chat.id, "🧠 Analyzing risk...");
...
🧠 ${explanation}`,
    ...
    console.error(err);
    bot.sendMessage(msg.chat.id, "⚠️ Unable to assess risk right now.");
  }
});
